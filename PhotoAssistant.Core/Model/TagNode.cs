...
namespace PhotoAssistant.Core.Model {
    public class TagNodeDataSource : IXtraSerializable {
        ...
        public virtual void SaveDataToXml(string xmlFile) => SaveLayoutCore(new XmlXtraSerializer(), xmlFile);
        public virtual void LoadDataFromXml(string xmlFile) => RestoreLayoutCore(new XmlXtraSerializer(), xmlFile);
        public virtual void LoadDataFromStream(System.IO.Stream stream) => RestoreLayoutCore(new XmlXtraSerializer(), stream);
        protected virtual bool SaveLayoutCore(XtraSerializer serializer, object path) {
            ...
            if(stream != null) {
                return serializer.SerializeObjects(GetXtraObjectInfo(), stream, GetType().Name);
            } else {
                return serializer.SerializeObjects(GetXtraObjectInfo(), path.ToString(), GetType().Name);
            }
        }
        protected virtual void RestoreLayoutCore(XtraSerializer serializer, object path) {
            System.IO.Stream stream = path as System.IO.Stream;
            if(stream != null) {
                serializer.DeserializeObjects(GetXtraObjectInfo(), stream, GetType().Name);
            } else {
                serializer.DeserializeObjects(GetXtraObjectInfo(), path.ToString(), GetType().Name);
            }
        }
        protected XtraObjectInfo[] GetXtraObjectInfo() {
            ArrayList result = new ArrayList();
            result.Add(new XtraObjectInfo(nameof(TagNodeDataSource), this));
