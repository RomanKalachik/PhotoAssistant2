...
namespace PhotoAssistant.Core.Model {
    ...
    public class DmModel {
        ...
        public static bool AllowGenerateDefaultTags {
            get; set;
        }
        static DmModel() => AllowGenerateDefaultTags = true;
        ...
        public bool AllowHierarchicallyKeywordFilters {
            get; set;
        }
        protected bool ShouldResetLastImportedFiltrer {
            get; set;
        }
        public void BeginAddFiles() => ShouldResetLastImportedFiltrer = true;
        ...
        string CreateThumb(string fileName) => fileName;
        public virtual IEnumerable<DmFile> GetActualGalleryDataSource() => GetFilteredFiles(FilterOperationType.Or);
        public virtual IEnumerable<DmFile> GetFiles() => Context.Files.Local.ToArray();
        string tabs = string.Empty;
        public void PrintFilterTree() {
            IOrderedEnumerable<Filter> res = from filter in Context.Filters.Local where filter.Parent == null orderby filter.Index ascending select filter;

            foreach(Filter f in res) {
                DebugPrintFilter(f);
            }
        }
        void DebugPrintFilter(Filter f) {
        ...
        public virtual IEnumerable<Filter> GetVisibleFilters() {
            ...
            IOrderedEnumerable<Filter> res = from filter in Context.Filters.Local where filter.Visible == true || filter.IsHeader orderby filter.Index ascending select filter;
            return res.ToList();
        }
        ...
        public List<DmStorageVolume> GetVolumes() => Context.Volumes.Local.ToList();
        ...
        public Filter GetFilter(Filter parent, object value) => parent.Children.FirstOrDefault((f) => object.Equals(f.Value, value));
        public Filter GetFilter(Filter parent, FilterType type, object value) => parent.Children.FirstOrDefault((f) => f.Type == type && object.Equals(f.Value, value));
        ...
        public Filter GetFilter(FilterType type) => Context.Filters.Local.FirstOrDefault((filter) => filter.Type == type);
        ...
        bool MatchFileByGrouping(DmFile file) => !file.IsGrouped || file.IsGroupOwner || file.GroupOwner.IsExpanded;
        FilterResult MatchPlaceFilter(DmFile file) => MatchFilterCore(file, PlaceFilter, (f, val) => f.Country == (string)val, (f, ft) => MatchStateFilter(f, ft));
        FilterResult MatchStateFilter(DmFile file, Filter filter) => MatchFilterCore(file, filter, (f, val) => f.State == (string)val, (f, ft) => MatchCityFilter(f, ft));
        FilterResult MatchCityFilter(DmFile file, Filter filter) => MatchFilterCore(file, filter, (f, val) => f.City == (string)val, (f, ft) => MatchLocationFilter(f, ft));
        FilterResult MatchLocationFilter(DmFile file, Filter filter) => MatchFilterCore(file, filter, (f, val) => f.Location == (string)val, null);
        FilterResult MatchEventFilter(DmFile file) => MatchFilterCore(file, EventsHeaderFilter, (f, val) => f.Event == (string)val, null);
        FilterResult MatchTagFilter(DmFile file) {
        ...
        FilterResult MatchTagFilterCore(DmFile file, Filter filter) => MatchFilterCore(file, filter, (f, val) => f.ContainsKeywordOrUnssigned((DmTag)val), (f, ft) => MatchTagFilterCore(f, ft));
        FilterResult MatchGenreFilter(DmFile file) {
            if(!AllowHierarchicallyKeywordFilters) {
        ...
        FilterResult MatchGenreFilterCore(DmFile file, Filter filter) => MatchFilterCore(file, filter, (f, val) => f.ContainsGenreOrUnssigned((DmTag)val), (f, ft) => MatchGenreFilterCore(f, ft));
        FilterResult MatchCollectionFilter(DmFile file) {
            if(!AllowHierarchicallyKeywordFilters) {
        ...
        FilterResult MatchCollectionFilterCore(DmFile file, Filter filter) => MatchFilterCore(file, filter, (f, val) => f.ContainsCollectionOrUnssigned((DmTag)val), (f, ft) => MatchCollectionFilterCore(f, ft));
        FilterResult MatchCategoryFilter(DmFile file) {
            if(!AllowHierarchicallyKeywordFilters) {
        ...
        FilterResult MatchCategoryFilterCore(DmFile file, Filter filter) => MatchFilterCore(file, filter, (f, val) => f.ContainsCategoryOrUnssigned((DmTag)val), (f, ft) => MatchCategoryFilterCore(f, ft));
        FilterResult MatchAutorFilter(DmFile file) {
            if(!AllowHierarchicallyKeywordFilters) {
        ...
        FilterResult MatchAutorFilterCore(DmFile file, Filter filter) => MatchFilterCore(file, filter, (f, val) => f.ContainsAutorOrUnssigned((DmTag)val), (f, ft) => MatchAutorFilterCore(f, ft));
        FilterResult MatchPeopleFilter(DmFile file) {
            if(!AllowHierarchicallyKeywordFilters) {
        ...
        FilterResult MatchPeopleFilterCore(DmFile file, Filter filter) => MatchFilterCore(file, filter, (f, val) => f.ContainsPeopleOrUnssigned((DmTag)val), (f, ft) => MatchPeopleFilterCore(f, ft));
        FilterResult MatchImportDateTimeFilter(DmFile file) => MatchFilterCore(file, ImportDateHeaderFilter, (f, val) => f.ImportDate.Year == (int)val, (f, ft) => MatchImportMonthFilter(f, ft));
        FilterResult MatchImportMonthFilter(DmFile file, Filter filter) => MatchFilterCore(file, filter, (f, val) => f.ImportDate.Month == (int)val, (f, ft) => MatchImportDayFilter(f, ft));
        FilterResult MatchImportDayFilter(DmFile file, Filter filter) => MatchFilterCore(file, filter, (f, val) => f.ImportDate.Day == (int)val, null);
        FilterResult MatchCreationDateTimeFilter(DmFile file) => MatchFilterCore(file, CreationDateHeaderFilter, (f, val) => f.CreationDate.Year == (int)val, (f, ft) => MatchCreationMonthFilter(f, ft));
        FilterResult MatchCreationMonthFilter(DmFile file, Filter filter) => MatchFilterCore(file, filter, (f, val) => f.CreationDate.Month == (int)val, (f, ft) => MatchCreationDayFilter(f, ft));
        FilterResult MatchCreationDayFilter(DmFile file, Filter filter) => MatchFilterCore(file, filter, (f, val) => f.CreationDate.Day == (int)val, null);
        FilterResult MatchColorLabelFilter(DmFile file) => MatchFilterCore(file, ColorLabelHeaderFilter, (f, val) => f.ColorLabel == val, null);
        FilterResult MatchRatingFilter(DmFile file) => MatchFilterCore(file, RatingHeaderFilter, (f, val) => f.Rating == (int)val, null);
        protected virtual FilterResult MatchMediaFormatTypeFilter(DmFile file) => MatchFilterCore(file, MediaFormatHeaderFilter, (f, val) => ((int)f.MediaFormat.Type) == ((int)val), (f, ft) => MatchMediaFormatFilter(ft, f));
        protected virtual FilterResult MatchMediaFormatFilter(Filter mediaTypeFilter, DmFile file) => MatchFilterCore(file, mediaTypeFilter, (f, val) => f.MediaFormat == val, null);
        protected virtual FilterResult MatchFolderFilter(DmFile file) => MatchFilterCore(file, FolderHeaderFilter, (f, val) => object.Equals(f.Folder, val), null);
        FilterResult CalcTotalRes(FilterResult totalRes, FilterResult res, FilterOperationType rootFilterType) {
        ...
        List<Filter> GetActiveFilters() {
            ...
            IQueryable<Filter> res = from filter in Context.Filters where filter.IsActive select filter;
            return res.ToList();
        }
        ...
        bool FileMatchLastImported(DmFile file) => file.ImportIndex == Properties.ImportIndex;
        int LockRaiseFilterValuesChangedCount {
            get; set;
        }
        ...
        void UpdateFilterByCategories(DmFile file, DmFile prevState, FileOperation operation) {
        ...
        void UpdateFilterByCollections(DmFile file, DmFile prevState, FileOperation operation) {
            if(operation == FileOperation.Add) {
        ...
        void UpdateFilterByGenres(DmFile file, DmFile prevState, FileOperation operation) {
            if(operation == FileOperation.Add) {
        ...
        void UpdateFilterByTags(DmFile file, DmFile prevState, FileOperation operation) {
            if(operation == FileOperation.Add) {
        ...
        void UpdateFilterByAutors(DmFile file, DmFile prevState, FileOperation operation) {
            if(operation == FileOperation.Add) {
        ...
        void UpdateFilterByPeoples(DmFile file, DmFile prevState, FileOperation operation) {
            if(operation == FileOperation.Add) {
        ...
        void RemoveZeroMatchFilters(Filter header) {
            for(int i = 0; i < header.Children.Count; ) {
                RemoveZeroMatchFilters(header.Children[i]);
                if(!header.Children[i].IsSystem && header.Children[i].MatchedCount == 0 && !header.Children[i].HasChildWithNonZeroMatchCount()) {
        ...
        void IncrementFilterPeoples(DmFile file) {
        ...
        void DecrementFilterPeoples(DmFile file) {
            if(file.HasPeoplesTag) {
        ...
        void IncrementFilterAutors(DmFile file) {
        ...
        void DecrementFilterAutors(DmFile file) {
            if(file.HasAutorsTag) {
        ...
        void IncrementFilterCategories(DmFile file) {
        ...
        void DecrementFilterCategories(DmFile file) {
            if(file.HasCategoriesTag) {
        ...
        void IncrementFilterCollections(DmFile file) {
        ...
        void DecrementFilterCollections(DmFile file) {
            if(file.HasCollectionsTag) {
        ...
        void IncrementFilterGenres(DmFile file) {
        ...
        void DecrementFilterGenres(DmFile file) {
            if(file.HasGenresTag) {
        ...
        void IncrementFilterKeywords(DmFile file) {
        ...
        void DecrementFilterKeywords(DmFile file) {
            if(file.HasKeywordsTag) {
        ...
        void IncrementFilter(Filter parentFilter, FilterType type, DmFile file, IEnumerable<IDmKeyword> values) {
            if(values == null) {
        ...
        void DecrementFilter(Filter parentFilter, FilterType type, IEnumerable<IDmKeyword> values) {
            if(values == null) {
        ...
        void DecrementTagFilterFlat(Filter parentFilter, FilterType type, DmTag tag, bool removeZeroMatchCount) {
        ...
        void DecrementFilter(Filter parentFilter, FilterType type, DmTag tag, bool removeZeroMatchCount) {
            if(!AllowHierarchicallyKeywordFilters) {
        ...
        protected bool ShouldRemoveFilter(Filter filter) => filter.MatchedCount == 0 && !filter.IsSystem && !filter.HasChildWithNonZeroMatchCount();
        void DecrementTagFilterHierarchical(Filter parentFilter, FilterType type, DmTag tag, bool removeZeroMatchCount) {
        ...
        Filter GetFilter(Filter root, FilterType type, List<DmTagNodeReversed> path, int endIndex, bool createIfNotExist) {
            Filter filter = root;
        ...
        void UpdateFilterByEvent(DmFile file, DmFile prevState, FileOperation operation) {
            if(operation == FileOperation.Add) {
        ...
        void UpdateFilterByMark(DmFile file, DmFile prevState, FileOperation operation) => UpdateFilterByMark(file, prevState == null ? false : prevState.Marked, operation);
        void UpdateFilterByReject(DmFile file, DmFile prevState, FileOperation operation) => UpdateFilterByRejected(file, prevState == null ? false : prevState.Rejected, operation);
        void UpdateFilterByRejected(DmFile file, bool prev, FileOperation operation) {
            if(operation == FileOperation.Add) {
        ...
        void UpdateFilterByMark(DmFile file, bool prev, FileOperation operation) {
            if(operation == FileOperation.Add) {
        ...
        void UpdateFilterByRating(DmFile file, int prevRating, FileOperation operation) {
            if(operation == FileOperation.Add) {
        ...
        void UpdateFilterByRating(DmFile file, DmFile prevState, FileOperation operation) => UpdateFilterByRating(file, prevState == null ? 0 : prevState.Rating, operation);
        void UpdateFilterByImportDate(DmFile file, DmFile prevState, FileOperation operation) {
            if(operation == FileOperation.Update) {
                return;
            }

            if(operation == FileOperation.Add) {
                IncrementFilter(ImportDateHeaderFilter, new FilterType[] { FilterType.ImportYear, FilterType.ImportMonth, FilterType.ImportDay }, new object[] { null, null, null }, new object[] { file.ImportDate.Year, file.ImportDate.Month, file.ImportDate.Day });
            } else {
                DecrementFilter(ImportDateHeaderFilter, new object[] { file.ImportDate.Year, file.ImportDate.Month, file.ImportDate.Day }, true);
            }
        }
        ...
        Filter IncrementFilter(Filter parentFilter, FilterType type, object value) => IncrementFilter(parentFilter, type, value, true);
        Filter IncrementFilter(Filter parentFilter, FilterType type, object value, bool increment) {
        ...
        Filter IncrementFilterTag(Filter parentFilter, FilterType type, DmFile file, DmTag tag) {
            Filter filter = null;
            if(!AllowHierarchicallyKeywordFilters) {
        ...
        void UpdateFilterByCreationDate(DmFile file, DmFile prevState, FileOperation operation) {
            if(operation == FileOperation.Add) {
                IncrementFilter(CreationDateHeaderFilter, new FilterType[] { FilterType.CreationYear, FilterType.CreationMonth, FilterType.CreationDay }, new object[] { null, null, null }, new object[] { file.CreationDate.Year, file.CreationDate.Month, file.CreationDate.Day });
            } else if(operation == FileOperation.Update) {
                if(prevState.CreationDate == file.CreationDate) {
                    return;
                }

                DecrementFilter(CreationDateHeaderFilter, new object[] { prevState.CreationDate.Year, prevState.CreationDate.Month, prevState.CreationDate.Day }, new object[] { file.CreationDate.Year, file.CreationDate.Month, file.CreationDate.Day }, true);
                IncrementFilter(CreationDateHeaderFilter, new FilterType[] { FilterType.CreationYear, FilterType.CreationMonth, FilterType.CreationDay }, new object[] { prevState.CreationDate.Year, prevState.CreationDate.Month, prevState.CreationDate.Day }, new object[] { file.CreationDate.Year, file.CreationDate.Month, file.CreationDate.Day });
            } else {
                DecrementFilter(CreationDateHeaderFilter, new object[] { file.CreationDate.Year, file.CreationDate.Month, file.CreationDate.Day }, true);
            }
        }
        void UpdateFilterByProject(DmFile file, DmFile prevState, FileOperation operation) {
            if(operation == FileOperation.Add) {
                IncrementFilter(ProjectHeaderFilter, new FilterType[] { FilterType.Client, FilterType.Project, FilterType.Scene }, new object[] { null, null, null }, new object[] { file.Client, file.Project, file.Scene });
            } else if(operation == FileOperation.Update) {
                if(prevState.Client == file.Client && prevState.Project == file.Project && prevState.Scene == file.Scene) {
                    return;
                }

                DecrementFilter(ProjectHeaderFilter, new object[] { prevState.Client, prevState.Project, prevState.Scene }, new object[] { file.Client, file.Client, file.Scene }, true);
                IncrementFilter(ProjectHeaderFilter, new FilterType[] { FilterType.Client, FilterType.Project, FilterType.Scene }, new object[] { prevState.Client, prevState.Project, prevState.Scene }, new object[] { file.Client, file.Project, file.Scene });
            } else {
                DecrementFilter(ProjectHeaderFilter, new object[] { file.Client, file.Project, file.Scene }, true);
            }
        }
        ...
        void RemoveFilterCore(Filter filter) {
        ...
        bool DecrementFilter(Filter filter, bool shouldRemoveWhenZeroMatchCount) {
            if(filter == null) {
                return false;
            }
        ...
        void UpdateFilterByMediaFormat(DmFile file, DmFile prevState, FileOperation operation) {
            if(file.MediaFormat == null) {
                return;
            }
        ...
        void UpdateFilterByColorLabel(DmFile file, DmFile prevState, FileOperation operation) => UpdateFilterByColorLabel(file, prevState == null ? null : prevState.ColorLabel, operation);
        void UpdateFilterByColorLabel(DmFile file, DmColorLabel prevLabel, FileOperation operation) {
            if(operation == FileOperation.Add) {
        ...
        void UpdateFilterByFolder(DmFile file, DmFile prevState, FileOperation operation) {
            if(operation == FileOperation.Update) {
                return;
            }
        ...
        void DecrementFilter(Filter parentFilter, object value, bool removeIfMatchedCountZero) {
        ...
        void RemoveFilterWithZeroMatchedCount(Filter filter, bool recursive) {
        ...
        void UpdateFilterByPlace(DmFile file, DmFile prevState, FileOperation operation) {
            if(operation == FileOperation.Add) {
                IncrementFilter(PlaceFilter, new FilterType[] { FilterType.Country, FilterType.State, FilterType.City, FilterType.Location }, new object[] { null, null, null, null }, new object[] { file.Country, file.State, file.City, file.Location });

                return;
            }

            if(operation == FileOperation.Remove) {
                DecrementFilter(PlaceFilter, new object[] { file.Country, file.State, file.City, file.Location }, true);
            ...
            if(operation == FileOperation.Update) {
                ...
                DecrementFilter(PlaceFilter, new object[] { prevState.Country, prevState.State, prevState.City, prevState.Location }, new object[] { file.Country, file.State, file.City, file.Location }, true);
                IncrementFilter(PlaceFilter, new FilterType[] { FilterType.Country, FilterType.State, FilterType.City, FilterType.Location }, new object[] { prevState.Country, prevState.State, prevState.City, prevState.Location }, new object[] { file.Country, file.State, file.City, file.Location });
            }
        }
        ...
        protected void DecrementFilter(FilterType rootType, object[] values, bool removeWhenZeroMatchCount) => DecrementFilter(GetFilter(rootType), values, removeWhenZeroMatchCount);
        ...
        protected void IncrementFilter(FilterType rootType, FilterType[] types, object[] prevValues, object[] values) => IncrementFilter(GetFilter(rootType), types, prevValues, values);
        ...
        Filter GetFilter(Filter filter, object[] values) {
            if(filter == null) {
                return null;
            }
        ...
        Filter GetFilter(FilterType filterType, object[] values) {
        ...
        public Filter GetFilter(FilterType filterType, object value) => Context.Filters.Local.FirstOrDefault((filter) => filter.Type == filterType && object.Equals(filter.Value, value));
        void UpdateFilterByImportedLastMonth(Filter filter, DmFile file, DmFile prevState, FileOperation operation) {
            if(operation == FileOperation.Update) {
                return;
            }
        ...
        void UpdateFilterByImportedLastWeek(Filter filter, DmFile file, DmFile prevState, FileOperation operation) {
            if(operation == FileOperation.Update) {
                return;
            }
        ...
        void UpdateFilterByImportedYesterday(Filter filter, DmFile file, DmFile prevState, FileOperation operation) {
            if(operation == FileOperation.Update) {
                return;
            }

            DateTime yesterday = DateTime.Now.Date.AddDays(-1);
            if(operation == FileOperation.Add && file.ImportDate.Year == yesterday.Year && file.ImportDate.Month == yesterday.Month && file.ImportDate.Day == yesterday.Day) {
                filter.MatchedCount++;
            } else if(file.ImportDate.Date == yesterday) {
                filter.MatchedCount--;
            }
        }
        void UpdateFilterByImportedToday(Filter filter, DmFile file, DmFile prevState, FileOperation operation) {
            if(operation == FileOperation.Update) {
                return;
            }

            if(operation == FileOperation.Add && file.ImportDate.Year == DateTime.Now.Year && file.ImportDate.Month == DateTime.Now.Month && file.ImportDate.Day == DateTime.Now.Day) {
                filter.MatchedCount++;
            } else if(file.ImportDate.Date == DateTime.Now.Date) {
                filter.MatchedCount--;
            }
        }
        void UpdateFilterByLastImported(Filter filter, DmFile file, DmFile prevState, FileOperation operation) {
            if(operation == FileOperation.Update) {
                return;
            }
        ...
        protected void RaiseFilterValuesChanged() {
            ...
            if(FilterValuesChanged != null) {
                FilterValuesChanged(this, EventArgs.Empty);
            }
        }
        string GetFilterTextByType(FilterType type, object value) => value.ToString();
        protected DateTime GetLastWeekStartDate() => DateTime.Now.AddDays(CultureInfo.CurrentCulture.DateTimeFormat.FirstDayOfWeek - DateTime.Now.DayOfWeek).Date;
        [Browsable(false)]
        public DmContext Context {
            get; set;
        }
        public bool OpenDataSource(string dataSource) => ConnectToDataSource(dataSource, false);
        public bool OpenDataSource(string dataSource, bool failIfMissing) => ConnectToDataSource(dataSource, failIfMissing);
        public bool CreateDataSource(string dataSource) => ConnectToDataSource(dataSource, false);
        ...
        public void LoadVolumes() => Context.Volumes.Load();
        void InitializeTagNodes() {
            Context.Tags.Load();
        ...
        void LoadFiles() => Context.Files.Load();
        ...
        protected bool ConnectToDataSource(string dataSource, bool failIfMissing) {
            ...
            sb.Version = 3;
            DbConnection connection = CreateConnection(dataSource, failIfMissing);
        ...
        static DbConnection CreateConnection(string filePath, bool failIfMissing) {
            ...
            return new SQLiteConnection { ConnectionString = new SQLiteConnectionStringBuilder { DataSource = filePath, FailIfMissing = failIfMissing, Version = 3 }.ConnectionString };
        }
        ...
        public bool CreateDefaultDataSource() => CreateDataSource(SettingsStore.Default.DefaultDataSourcePath);
        public Version GetDataSourceVersion() => new Version();
        ...
        public object GetFilterRootId() => Guid.Empty;
        DbPropertiesModel PropertiesCore {
            get; set;
        }
        ...
        [Browsable(false)]
        public DmFile FileBeforeUpdate {
            get; set;
        }
        public void BeginUpdateFile(DmFile fileInfo) => FileBeforeUpdate = fileInfo == null ? null : fileInfo.Clone();
        ...
        void UngroupFile(DmFile groupOwner) {
            if(!groupOwner.IsGroupOwner) {
                return;
            }

            groupOwner.IsGroupOwner = false;
            IEnumerable<DmFile> res = from file in Context.Files.Local where file.GroupId == groupOwner.Id select file;
        ...
        void SetGroupExpanded(DmFile file, bool isExpanded) => file.IsExpanded = isExpanded;
        void ToggleGroupExpand(DmFile groupOwner) {
            if(!groupOwner.IsGroupOwner) {
                return;
            }
        ...
        IDmKeyword CreateKeyword(DmTag tag, TagType tagType) {
            IDmKeyword res = null;
            switch(tagType) {
                case TagType.Autor:
                    res = new DmAutor();
        ...
        public void RemoveKeyword(DmFile file, DmTag tag, TagType tagType) => file.RemoveTag(this, tag, tagType);
        int updateCount = 0;
        ...
        public void CancelUpdate() => UpdateCount--;
        protected bool IsLockUpdate => UpdateCount > 0;
        ...
        void RemoveKeywords(DmFile file) {
        ...
        void RemoveKeywords(DmFile file, ICollection<DmPeople> collection) {
            if(collection == null) {
                return;
            }
        ...
        void RemoveKeywords(DmFile file, ICollection<DmKeyword> collection) {
            if(collection == null) {
                return;
            }
        ...
        void RemoveKeywords(DmFile file, ICollection<DmGenre> collection) {
            if(collection == null) {
                return;
            }
        ...
        void RemoveKeywords(DmFile file, ICollection<DmCollection> collection) {
            if(collection == null) {
                return;
            }
        ...
        void RemoveKeywords(DmFile file, ICollection<DmCategory> collection) {
            if(collection == null) {
                return;
            }
        ...
        void RemoveKeywords(DmFile file, ICollection<DmAutor> collection) {
            if(collection == null) {
                return;
            }
        ...
        public CriteriaOperator ApplyFiltration() => null;
        ...
        void SilentClearTags(DbSet<DmTag> tags) => tags.Local.ToList().ForEach(t => Context.Entry(t).State = EntityState.Detached);
        public DmTag GetTag(string value) => GetTag(value, TagType.Tag);
        public DmTag GetTag(string value, TagType type) => Context.Tags.Local.FirstOrDefault((t) => t.Value == value && t.Type == type);
        ...
        void FillSuggestedTags(ICollection<DmTagNodeReversed> nodes, List<DmTagNodeReversed> suggestList) {
            if(nodes == null) {
                return;
            }
    ...
    public class DmModelHelpers {
        public DmModelHelpers(DmModel model) => Model = model;
